...
- **Docker Compose** with the app and the parent bundle mounted as volumes (`../..` → `/var/password-toggle-bundle`).
...
There are demos for **Symfony 7**, **8** and **8 (PHP 8.5)** (e.g. **demo/symfony7**, **demo/symfony8**, **demo/symfony8-php85**). Each has its own Dockerfile, docker-compose.yml and Makefile. From the bundle root you run e.g. `make -C demo/symfony8 up` (see the demo's README for the URL and port). Dockerfile, docker-compose.yml and Makefile. From the bundle root you run e.g. `make -C demo/symfony8 up` (see the demo's README for the URL and port).
